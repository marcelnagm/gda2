...
class AlunoMatriculaForm extends BaseTbalunomatriculaForm {
    ...
    private $URLPost = 'admin/NovoAluno';
...
//        $this->widgetSchema['matricula']->setAttribute('readonly','readonly');
        ...
        $tipos = array(
            ...
            '39' => 'ENEM SISU (AUTODECLARADO)',
            '37' => 'ENEM SISU (RENDA AUTODECLARADO)',
            '38' => 'ENEM SISU (ESCOLA PÚBLICA AUTODECLARADO)',
        ...
        if (sfContext::getInstance()->getUser()->getAttribute('import')) {
            ...
            $this->widgetSchema['id_versao_curso'] = new sfWidgetFormChoice(array(
                        ...
                        'multiple' => false,
                        'label' => 'Curso',
                        'choices' => array($aluno->getIdVersaoCurso() => $aluno->getTbcursoversao())
                    ));
            $this->widgetSchema['id_tipo_ingresso'] = new sfWidgetFormChoice(array(
                        'expanded' => false,
                        'multiple' => false,
                        'choices' => array($aluno->getIdTipoIngresso() => $aluno->getTbtipoingresso()),
                        'label' => 'Tipo de Ingresso',
                    ));

            $this->widgetSchema['id_polo'] = new sfWidgetFormChoice(array(
                        'choices' => array(
                            $aluno->getIdPolo() => $aluno->getTbPolos()
                        ),
                        'expanded' => false,
                        'multiple' => false,
                        'label' => 'Pólo'
