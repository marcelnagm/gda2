-- Function: checa_chq_horario(integer, bigint)

-- DROP FUNCTION checa_chq_horario(integer, bigint);
...
FOR lsh IN SELECT h.id_oferta, h.dia, h.hora_inicio, h.hora_fim, o.id_periodo, o.dt_inicio, o.dt_fim
...
--                    hora_inicio, hora_fim, dia
		...
		AND
			...
			AND
			    (
				( dt_inicio >= lsh.dt_inicio AND dt_inicio <= lsh.dt_fim )
			      OR
                                ( dt_fim >= lsh.dt_inicio AND dt_fim <= lsh.dt_fim )
                            )
                        AND 
                            (
