--alteração do algoritmo , prevendo quando ja existe aluno matriculado nesta oferta,
	...
	ELSE
		...
		UPDATE tbfila SET id_situacao = 5 WHERE id_fila=lsf.id_fila;
			...
			-- se for para oferta de calouros
				...
				--fase 2 para CAL
					 IF ( _vagas = 0 ) THEN
					        --nao eh calouro e nao tem vaga
						IF (checa_calouro(lsf.matricula,ls_oferta.id_periodo) = false) THEN
						-- set turma lotada
							UPDATE tbfila SET id_situacao = 5 WHERE id_fila=lsf.id_fila;
						ELSE
						-- caso seja calouro fica aceito
						UPDATE tbfila SET id_situacao = 1 WHERE id_fila=lsf.id_fila;			     	  		  END IF;
					 ELSE
					 -- marca como aceito
					    UPDATE tbfila SET id_situacao = 1 WHERE id_fila=lsf.id_fila;
					    _vagas = _vagas - 1;
