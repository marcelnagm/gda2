-- Function: get_ch_eletiva(bigint, integer, boolean)

-- DROP FUNCTION get_ch_eletiva(bigint, integer, boolean);

CREATE OR REPLACE FUNCTION get_ch_obr(bigint, integer, boolean)
  RETURNS integer AS
$BODY$DECLARE
...
WHERE
  ...
  tbdisciplina.situacao = 'ATIVA' AND matricula = Amatricula) As A where A.car = 'OBRIGATORIA'
) as B where
...
END;$BODY$
  ...
  COST 100;
ALTER FUNCTION get_ch_obr(bigint, integer, boolean) OWNER TO postgres;
