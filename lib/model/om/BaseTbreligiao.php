...
/**
 * Base class that represents a row from the 'tbreligiao' table.
 *
 * 
 *
 ...
 */
abstract class BaseTbreligiao extends BaseObject  implements Persistent {
	 ...
	 * @return     Tbreligiao The current object (for fluent API support)
	 */
	public function setIdReligiao($v)
	{
		...
		if ($this->id_religiao !== $v) {
			$this->id_religiao = $v;
			$this->modifiedColumns[] = TbreligiaoPeer::ID_RELIGIAO;
		}
	...
	public function setDescricao($v)
	{
		...
		if ($this->descricao !== $v) {
			$this->descricao = $v;
			$this->modifiedColumns[] = TbreligiaoPeer::DESCRICAO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_religiao = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = TbreligiaoPeer::NUM_COLUMNS - TbreligiaoPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Tbreligiao object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TbreligiaoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTbreligiao:delete:pre') as $callable)
			{
			...
			if ($ret) {
				TbreligiaoPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTbreligiao:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTbreligiao:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTbreligiao:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				$con->commit();
				TbreligiaoPeer::addInstanceToPool($this);
				return $affectedRows;
			}
		} catch (PropelException $e) {
			$con->rollBack();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = TbreligiaoPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
										 // should always be true here (even though technically
										 // BasePeer::doInsert() can insert multiple rows).

					$this->setIdReligiao($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += TbreligiaoPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TbreligiaoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = TbreligiaoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = TbreligiaoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getIdReligiao(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdReligiao($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TbreligiaoPeer::ID_RELIGIAO)) $criteria->add(TbreligiaoPeer::ID_RELIGIAO, $this->id_religiao);
		if ($this->isColumnModified(TbreligiaoPeer::DESCRICAO)) $criteria->add(TbreligiaoPeer::DESCRICAO, $this->descricao);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(TbreligiaoPeer::ID_RELIGIAO, $this->id_religiao);
	...
	/**
	 * Generic method to set the primary key (id_religiao column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Tbreligiao (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIdReligiao(NULL); // this is a auto-increment column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Tbreligiao Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     TbreligiaoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new TbreligiaoPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this Tbreligiao has previously been saved, it will retrieve
	 * related Tbalunobackups from storage. If this Tbreligiao is new, it will return
		...
		if ($this->collTbalunobackups === null) {
			if ($this->isNew()) {
			   $this->collTbalunobackups = array();
				...
				$criteria->add(TbalunobackupPeer::ID_RELIGIAO, $this->id_religiao);
	 ...
	 * Otherwise if this Tbreligiao is new, it will return
	 * an empty collection; or if this Tbreligiao has previously
	 * been saved, it will retrieve related Tbalunobackups from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Tbreligiao.
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseTbreligiao:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseTbreligiao::%s', $method));
	  }
