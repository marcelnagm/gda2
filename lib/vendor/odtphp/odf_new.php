...
class Odf {
   ...
   // To store content of META-INF/manifest.xml file
    ...
    public function __construct($filename, $config = array()) {
        ...
        if ($this->config['PATH_TO_TMP'])
            $this->tmpdir = preg_replace('|[\/]$|', '', $this->config['PATH_TO_TMP']); // Remove last \ or /
 $this->tmpdir .= ( $this->tmpdir ? '/' : '') . $md5uniqid;
        ...
        if (($this->stylesXml = $this->file->getFromName('styles.xml')) === false) {
            throw new OdfException("Something is wrong with styles.xml - Check that such file is correctly formed in source file '$filename'");
        }
        $this->file->close();
    ...
    public function setVars($key, $value, $encode = true, $charset = 'ISO-8859') {
        ...
        if (strpos($this->contentXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) === false && strpos($this->stylesXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) === false) {
